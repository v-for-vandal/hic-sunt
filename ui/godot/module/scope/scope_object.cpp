...
void ScopeObject::_bind_methods() {
  ...
  ClassDB::bind_static_method("ScopeObject", D_METHOD("create_scope"), &ScopeObject::create_scope);
}
...
bool ScopeObject::add_numeric_modifier(const StringName& variable, const StringName& key, float add,
                                       float mult) {
...
bool ScopeObject::add_string_modifier(const StringName& variable, const StringName& key,
  ...
  return static_cast<bool>(scope_->SetStringModifier(variable, key, value, level));
}
...
Dictionary ScopeObject::explain_all() {
  ...
  auto add_numeric_element_fn = [&result](StringName scope_id, StringName variable,
    ...
    static_cast<Dictionary>(static_cast<Dictionary>(result.get_or_add(variable, Dictionary{}))
  ...
  scope_->ExplainAllVariables(hs::utils::Overloaded{add_numeric_element_fn, add_string_element_fn});
  return result;
}

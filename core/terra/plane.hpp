...
public:
  ...
  Plane(
    ...
    int region_radius,
    // external_region_radius is the radius of the region from the point of view
    // of global map. It is used primarily when calculating 'real-world'
    // distances for world generation, or some effect that should degrade
    // with distance to its source.
    // The area is not rendered, so you can put almost any value here, however
    // large values will lead to weird planes.
    // Hexagon defined by external areas touch each other. The distance between
    // centers of two neighbouring hexagons is 2*external radius
    // If less then region_radius, or negative, we will fallback to some
    // reasonable value
    // This parameter has no effect on off-surface regions

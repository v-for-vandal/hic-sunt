...
Surface<Cell, CoordinateSystem>::Surface(SurfaceShape shape)
    : shape_(shape), allocation_size_(GetAllocationSize(shape)),
      data_size_(allocation_size_.first * allocation_size_.second),
      data_storage_(new Cell[data_size_]),
      cells_(CellsArrayView<Cell>(data_storage_.get(), allocation_size_.first,
                                  allocation_size_.second),
             shape) {}

template <typename Cell, typename CoordinateSystem>
bool SurfaceView<Cell, CoordinateSystem>::operator==(
    const SurfaceView &other) const {
  if (this == &other) {
    return true;
  }
...
template <typename Cell, typename CoordinateSystem>
std::pair<size_t, size_t> Surface<Cell, CoordinateSystem>::GetAllocationSize(
    const SurfaceShape &shape) noexcept {
  auto box = shape.BoundingBox();

  return std::make_pair(size_t(box.q_size().ToUnderlying()),
                        size_t(box.r_size().ToUnderlying()));
}

template <typename Cell, typename CoordinateSystem>
inline Surface<Cell, CoordinateSystem>
ParseFrom(const auto &proto_source,
          serialize::To<Surface<Cell, CoordinateSystem>>) {
  SurfaceShape<CoordinateSystem> shape = ParseFrom(
      proto_source.shape(), serialize::To<SurfaceShape<CoordinateSystem>>{});

  Surface<Cell, CoordinateSystem> result{shape};

  const auto &cells_handle = proto_source.cells();
  if (static_cast<int>(result.data_size()) < cells_handle.size()) {
    spdlog::error("Not enough storage in Surface. Surface data size: {} proto "
                  "cells size: {}",
                  result.data_size(), cells_handle.size());
...
template <typename Cell, typename CoordinateSystem>
auto SerializeTo(const Surface<Cell, CoordinateSystem> &source,
                 auto &proto_destination) {
